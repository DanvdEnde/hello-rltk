...
impl ParticleBuilder {
    ...
    pub fn request(
        ...
        self.request.push(ParticleRequest {
            x,
            y,
            fg,
            bg,
...
impl<'a> System<'a> for ParticleSpawnSystem {
    ...
    fn run(&mut self, data: Self::SystemData) {
        ...
        for new_particle in particle_builder.request.iter() {
        ...
        particle_builder.request.clear();
    }
}
...
pub fn cull_dead_particle(ecs: &mut World, ctx: &Rltk) {
    ...
    for dead in dead_particles.iter() {
        ecs.delete_entity(*dead).expect("Particle refuses to die");
    }
}
